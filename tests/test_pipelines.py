...
class PipelineTests(unittest.TestCase):
    def setUp(self):
        ...
        self.compiler = types.ModuleType("shipmblang._compiler")
    ...
    def test_adapter_retains_contract_and_disables_memory_by_env(self):
        provider = Mock()
        with patch.dict(sys.modules, {"shipmblang._compiler": self.compiler}), patch.dict(os.environ, {"SHIPMB_MEMORY": "off", "SHIPMB_MEMORY_DB": "custom.sqlite"}):
    ...
    def test_direct_defaults_to_bytecode(self):
        with patch.dict(sys.modules, {"shipmblang._compiler": self.compiler}):
    ...
    def test_general_run_uses_validated_compiler_runtime(self):
        self.artifact.update(version="0.3", profile="general")
        runtime = types.ModuleType("shipmblang._compiler.runtime")
        runtime.run_artifact = Mock(return_value=({"output": ["27"]}, []))
        with patch.dict(sys.modules, {"shipmblang._compiler": self.compiler, "shipmblang._compiler.runtime": runtime}):
    ...
    def test_compile_only_general_never_loads_runtime(self):
        with patch.dict(sys.modules, {"shipmblang._compiler": self.compiler, "shipmblang._compiler.runtime": None}):
    ...
    def test_older_compiler_keeps_roku_default_and_rejects_general_clearly(self):
        ...
        with patch.dict(sys.modules, {"shipmblang._compiler": self.compiler}):
            ...
            with self.assertRaisesRegex(pipelines.CompilerUnavailableError, "bundled compiler does not support the general profile"):
    ...
    def test_legacy_explicit_database_precedence(self):
        ...
        memory = types.ModuleType("shipmblang._compiler.memory")
        memory.MemoryStore = Mock()
        with patch.dict(sys.modules, {"shipmblang._compiler.memory": memory}):
    ...
    def test_clarification_is_visible_and_never_falls_back(self):
        ...
        output = io.StringIO()
        with patch.dict(sys.modules, {"shipmblang._compiler": self.compiler}), patch.object(sys, "argv", ["compile", "--pipeline", "direct", "Use it."]), contextlib.redirect_stdout(output):
            with self.assertRaises(SystemExit) as failure:
                pipelines.main()
        self.assertEqual(failure.exception.code, 1)
    ...
    def test_missing_bundle_has_actionable_error(self):
        with patch.dict(sys.modules, {"shipmblang._compiler": None}):
            with self.assertRaisesRegex(pipelines.CompilerUnavailableError, r"Reinstall ShipMBLang"):
    ...
    def test_bytecode_output_keeps_warnings_visible(self):
        ...
        stderr = io.StringIO()
        with patch.dict(sys.modules, {"shipmblang._compiler": self.compiler}), contextlib.redirect_stderr(stderr):
    ...
    def test_default_compiles_broader_english_with_general_profile(self):
        with patch.dict(sys.modules, {"shipmblang._compiler": self.compiler}), patch.dict(os.environ, {"SHIPMB_MODEL_PROVIDER": "openai_compatible"}):
    ...
    def test_model_frontend_is_lazy_and_uses_existing_chat_provider(self):
        frontend_module = types.ModuleType("shipmblang._compiler.english_model")
        frontend_module.EnglishModelFrontend = Mock(return_value=lambda source: {"paraphrase": "Show 9."})
        provider = object()
        with patch.dict(sys.modules, {"shipmblang._compiler.english_model": frontend_module}), patch("shipmblang.providers.load_chat_provider", return_value=provider) as load:
    ...
    def test_missing_model_returns_actionable_question(self):
        ...
        with patch.dict(sys.modules, {"shipmblang._compiler.english_model": frontend_module}), patch("shipmblang.providers.load_chat_provider", return_value=None):
    ...
    def test_legacy_captures_original_text_and_result(self):
        ...
        memory.MemoryStore = Mock()
        source = "  Use ShipMB.\n"
        with patch.dict(sys.modules, {"shipmblang._compiler.memory": memory}), patch.dict(os.environ, {"SHIPMB_MEMORY": "on"}):
    ...
    def test_legacy_failed_compile_is_captured(self):
        ...
        memory = types.ModuleType("shipmblang._compiler.memory")
        memory.MemoryStore = Mock()
        with patch.dict(sys.modules, {"shipmblang._compiler.memory": memory}), patch.dict(os.environ, {"SHIPMB_MEMORY": "on"}), patch.object(natural_syntax, "_compile_natural_program", side_effect=ValueError("bad source")):
    ...
    def test_memory_failure_does_not_discard_compile(self):
        from shipmblang import compile_natural_program
        with patch.dict(sys.modules, {"shipmblang._compiler.memory": None}), patch.dict(os.environ, {"SHIPMB_MEMORY": "on"}), warnings.catch_warnings(record=True) as caught:
...
class BlockLegacy(importlib.abc.MetaPathFinder):
    def find_spec(self, fullname, path=None, target=None):
        if fullname == 'driplm' or fullname.startswith('driplm.') or fullname in {
            'shipmblang.providers', 'shipmblang.runtime_context', 'shipmblang._compiler.runtime',
            'shipmblang._compiler.ir', 'shipmblang._compiler.optimizer', 'shipmblang._compiler.core'
        }:
            raise AssertionError(fullname)
sys.meta_path.insert(0, BlockLegacy())
compiler = types.ModuleType('shipmblang._compiler')
...
sys.modules['shipmblang._compiler'] = compiler
