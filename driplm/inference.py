...
class DripInference:
    def __init__(self, checkpoint_path, tokenizer_path, device="cpu"):
        ...
        if os.path.exists(config_path):
            ...
            self.config = DripConfig(**{k: v for k, v in ckpt["config"].items() if k in valid_fields})
        else:
            print("Warning: No config found, using defaults")
            self.config = DripConfig()

        self.model = DripLM(self.config).to(self.device)
        ...
        print(f"DripLM loaded: {total/1e6:.1f}M params")
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Chat with Drip")
    ...
    engine = DripInference(args.checkpoint, args.tokenizer, args.device)
